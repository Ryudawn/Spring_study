...
public class MemberServiceImpl implements MemberService { //서비스 인터페이스 상속받기
	...
	@Override
	public Page<MemberDTO> getList(int page) {  //페이지 번호 받기
		int pageNum = (page == 0) ? 0 : page - 1; //page index는 0부터 시작
		Pageable pageable = PageRequest.of(pageNum, 10, Sort.by("regDate").descending());  //페이지번호, 회원데이터건수, 정렬방법을 입력하여 페이징 조건 생성
